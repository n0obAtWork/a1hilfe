# Archiv - Vorschau

<!-- source: https://amic.de/hilfe/_archivvorschau.htm -->

Die Dokumentenverwaltung bietet in allen Auswahllisten, in denen die Schlüsselbegriffe des Archivs (fa_id, fa_mndnr) verfügbar sind, Vorschauen für die gängigen Mime-Typen resp. Formate.

Welche das genau sind entnehme man der Anwendung „Mime“, Variante „Mime“ (**[MIME]**) der dortigen Spalte „Archiv-Vorschau“.

Das A.eins-System unterstützt folgende Typen der Vorschau, diese sind mit Kontext-Menüs ausgestattet, die speziell auf den Typen ausgerichtete Funktionen anbieten.

| Vorschau-Typ | Anmerkung |
| --- | --- |
| Bild-Anzeige | Die Bild-Anzeige bietet im Kontext-Menü folgende Darstellungsarten. Diese werden sich sitzungsübergreifend für den jeweiligen A.eins-Benutzer gemerkt.<br> <br>• Dehnen<br>Das Bild wird gestreckt oder verkleinert, damit es der Größe der Bild-Anzeige entspricht.<br>• Originalgröße<br>Das Bild wird in Original-Größe dargestellt. Ist das Bild größer als die Bild-Anzeige, dann wird die Bild-Anzeige mit Rollbalken ausgestattet.<br>• Skalieren<br>Das Bild wird unter Beibehaltung der Größenverhältnisse vergrößert oder verkleinert.<br> |
| PDF-Anzeige | Bietet diverse Anzeige- und Recherche-Funktionen für das betreffende PDF-Dokument. |
| Text-Anzeige | Keine besonderen aktive Funktionen. |
| E-Mail-Anzeige | Bietet neben der Vorschau der E-Mail noch Anzeige von weiteren E-Mail-Informationen (z.B. Absender, Anhangs-Übersicht, usw.) |
