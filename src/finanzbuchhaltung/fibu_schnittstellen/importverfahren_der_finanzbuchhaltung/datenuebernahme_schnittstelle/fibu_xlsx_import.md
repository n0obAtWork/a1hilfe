...
```sql
create procedure p_dueb_xlsx  ( in in_fa_id integer , in in_fa_mndnr integer )
  result(
...
begin
  ...
  set dc_data = (select FA_XmlErweiterung from formulararchiv where fa_id = in_fa_id and fa_mndnr = in_fa_mndNr);
--Hier beginnt der Individuelle Teil:
  ...
  from openxml( dc_data, '//Excel/Worksheets/Worksheet/Rows/Row')
  with
  (
    zeile             integer   '@idx',
    ident             char(10)  './A' ,
    klKurz            char(20)  './D' ,
    fibuv_fremdnr     char(255) './E' ,
  ...
  as daten
  where zeile > 1;                         -- ggf. die Überschriftszeie überlesen
-- Hier endet der Individuelle Teil:
  select
    ...
    poszaehler,
    fibuv_klasse,
    fibuv_herktyp,                                     --Ist der Herkunftstyp nicht angegeben, wird 30 für Import eingetragen. Eigene Herkunftstypen sollten ab 100 beginnen
    fibuv_fremdnr,
    numkreisnummer,
    fibuv_numnummer,
    fibuv_nummer,
