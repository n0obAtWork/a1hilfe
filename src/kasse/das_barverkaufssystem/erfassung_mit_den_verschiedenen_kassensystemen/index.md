# Erfassung mit den verschiedenen Kassensystemen

<!-- source: https://amic.de/hilfe/erfassungmitdenverschiedenenka.htm -->

In A.eins gibt es verschiedene Arten, einen Barvorgang zu bearbeiten.

Die Unterscheidung wird hier getroffen zwischen:

[POS Kasse](./pos_kasse/index.md) (veraltet – wird nicht mehr gepflegt)

[Tresenkasse](./tresenkasse/index.md)

[Marktkasse](./marktkasse/index.md)

[Zahlung](../zahlung_in_den_verschiedenen_kassensystemen/zahlungen_mit_der_klassischen_zahlungsmaske/index.md)

Achtung:

Ein Wechsel des Barverkaufs in eine Rechnung ist nur sinnvoll, wenn vor der Erfassung des Positionsteiles die Kundennummer angegeben wurde.

<p class="siehe-auch">Siehe auch:</p>

- [POS-Kasse](./pos_kasse/index.md)
- [Tresenkasse](./tresenkasse/index.md)
- [Marktkasse](./marktkasse/index.md)
