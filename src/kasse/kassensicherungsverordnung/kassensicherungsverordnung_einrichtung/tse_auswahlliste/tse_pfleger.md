...
| Erläuterungen zu nachfolgenden Infos | \- ***HasPassedSelftest***: Die TSE hat einen internen Selbsttest ausgeführt.<br>\- ***IsErsInterfaceActive***: Gibt an, ob die TSE-Transaktionen ausführen kann, oder nicht.<br>\- ***HasValidTime***: Gibt an, ob in der TSE ein Datum gesetzt ist.<br>(Das Datum wird immer in *Greenwich Mean Time (UTC)* angegeben)<br>\- ***isDecommisioned***: Zeigt an, ob die TSE stillgelegt wurde. |
