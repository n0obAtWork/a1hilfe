# eRechnung – im Archiv

<!-- source: https://amic.de/hilfe/erechnungimarchiv.htm -->

Im Archiv gibt es für eRechnung eine eigene Variante.

In das Archiv kann [manuell importiert](./manueller_import.md) werden oder mit [[EMAIL]](../../zusatzprogramme/tammo/index.md) oder [[FAI].](../../dokumentenverwaltung/archiv_import/archiv_import_stammdatenpfleger_formulararchiv_importe/index.md)

In dieser Variante werden folgende Datentypen angezeigt:

<div class="table-wrapper">
  <table>
    <tbody>
      <tr>
        <td colspan="2">
          <p><strong>Belegklassen XML</strong></p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8040</strong></p>
        </td>
        <td>
          <p>Dies ist das exportierte Xml.</p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8041</strong></p>
        </td>
        <td>
          <p>Das ist die HTML-Visualisierung des exportierten Xml</p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8042</strong></p>
        </td>
        <td>
          <p>Der eRechnungsexport ist fehlerhaft.</p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8049</strong></p>
        </td>
        <td>
          <p>Das ist der eRechnungsexport mit ZUGFerD (<strong><em>ab Herbstversion 2025</em></strong>)</p>
        </td>
      </tr>
    </tbody>
  </table>
</div>

<div class="table-wrapper">
  <table>
    <tbody>
      <tr>
        <td colspan="2">
          <p><strong>Belegklassen Import</strong></p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8044</strong></p>
        </td>
        <td>
          <p>Dies ist ein ZUGFerD-Pdf mit eingebetteter Xml-Datei. (<b><i>ab Herbstversion 2025</i></b>)</p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8045</strong></p>
        </td>
        <td>
          <p>Dies ist das importierte Xml.</p>
        </td>
      </tr>
      <tr>
        <td>
          <p><strong>Belegklasse 8046</strong></p>
        </td>
        <td>
          <p>Das ist die HTML-Visualisierung des Xml</p>
        </td>
      </tr>
    </tbody>
  </table>
</div>

<p class="siehe-auch">Siehe auch:</p>

- [Manueller Import](./manueller_import.md)
