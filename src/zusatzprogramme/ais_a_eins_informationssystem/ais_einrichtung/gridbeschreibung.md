...
| Anzeigefeld | Die Daten werden nur angezeigt, können jedoch **NICHT** geändert werden. Ist ein Control (s.u.) hinterlegt, kann das Feld mit der Maus ausgewählt bzw. ein Doppelklick auf diesem Feld ausgeführt werden.<br><br><br><strong>Achtung:</strong> <em>Ist die</em> [*Datenherkunft*](./datenbeschreibung.md) ***Relation**, so wird das Feld trotzdem mit gespeichert.*<br> |
